...
if __name__ == "__main__":
    ...
    #this is a map transformation on line. separate them into a tuple. 0.00000017 is value of (1/n) 
    ...
    for iteration in range(int(sys.argv[2])):
        ...
        init_ranks = combine.reduceByKey(add).mapValues(lambda init_ranks: init_ranks)
	final_rank=init_ranks.map(lambda init_ranks_line: (int(init_ranks_line[0]),init_ranks_line[1]))
    
    
    #Get text fiel and init spark context & form tuple
